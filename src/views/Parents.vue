<template >
    <section id="parents">
        <div class="container">
            <div class="section-label">Родители</div>
            ...
            <div class="parents-list" v-if="parents && parents.length > 0">
                <div class="parents-list__item parents-list__head">
                    <span>ФИО</span>
                    <span>Дети</span>
                    <span>Контакты</span>
                </div>
                <div class="parents-list__item" v-for="(item) in parents" :key="item.id">
            ...
            <div class="empty" v-else>
                Вы пока не добавили ни одного родителя
            </div>
        </div>
    </section>
</template>
...
export default {
    ...
    computed: {
        parents() {
            return useParentsStore().parentsList
        }
    },
    methods: {
        async deleteUser(id) {
            useParentsStore().deleteParent(id)
        }
    },
    created() {
        useParentsStore().getAllParents()
    },
